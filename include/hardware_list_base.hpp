...
namespace ListBase {
    ...
    template<typename Rt, typename Key, typename ...Args>
    std::function<Rt(const Key &, Args &&...)> get_callback(std::map<Key, std::function<Rt(Args...)>> &callback_map) {
    ...
    class List_base<idx, Head> {
       public:
        ...
        virtual void register_callback(const CallbackKey &key, const CallbackFun &fun) {
            callback_map[key] = fun;
        }
    };
    ...
    void send(List_base<idx, Head, Tail...> &list, Args &&... args) {
        list.send(args...);
    }
    ...
    template<size_t idx, typename Head, typename ...Tail>
    void register_callback(List_base<idx, Head, Tail...> &list,
